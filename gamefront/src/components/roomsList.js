    ...
    return(
        <>
            <div className="list-group">
                ...
                {!player.room &&
                    <div>
                        {Object.keys(rooms).map((key) => <div key={key} className="list-item">{rooms[key].name} <button onClick={() => {joinRoom(key)}} disabled={rooms[key].player1 && rooms[key].player2}>Entrar na Sala</button></div>)}
                    </div>
                }
